...
object Ex5Main extends App {
  ...
  val words: Long = articlesRdd.map(str=>str.split(" ").length).sum().toLong
  ...
  val chars: Long = articlesRdd.map(str=>str.replace(" ", "").toCharArray().length).sum().toLong
  println(s"#chars = ${chars}")

  val numChars: Long = articlesRdd.flatMap(_.toCharArray().map(_.toString)).filter(_.matches("[0-9]")).count()
  ...
  val words5Count: Long = articlesRdd.flatMap(_.split(" ").filter(word => (word.length==5  && !word.matches("DisCo")))).count()
  println(s"5-character words: ${words5Count}")

  val commonWordArr = articlesRdd.flatMap(
    _.split(" ").filter(word=>(word.length==5 && !word.matches("DisCo")))
  ).map(str=>(str,1)).countByKey().toSeq.sortWith((l,r)=>l._2>r._2).head
  val commonWord: String = commonWordArr._1
  val commonWordCount: Int = commonWordArr._2.toInt
  ...
  val distinctPrimes: List[Int] = factorRdd.map(v=>v._2).reduce((l,r)=>(l:::r)).distinct.sortWith((l,r)=>l<r)
  ...
  val lyricsCount = lyricsRdd.flatMap(line => line.split(" ")) // split each row in the RDD on a blank space character
                             .map(word => (word, 1)) // create a tuple with the word and its count (i.e. 1)
                             .reduceByKey((v1, v2) => v1 + v2)  // reduce the tuple by its key (i.e. 'word') and sum the counts
